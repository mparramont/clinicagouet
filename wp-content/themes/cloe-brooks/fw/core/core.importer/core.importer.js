...
jQuery(document).ready(function(){
	...
	jQuery('.trx_importer_section').on('click', '.trx_buttons input[type="button"]', function() {
		...
		CLOE_BROOKS_STORAGE['importer_error_messages'] = '';
		CLOE_BROOKS_STORAGE['importer_ignore_errors'] = true;
		cloe_brooks_importer_do_action(steps, 0);
	});
});

// Call specified action (step)
function cloe_brooks_importer_do_action(steps, idx) {
	"use strict";
	...
	// AJAX query params
	var data = {
		ajax_nonce: CLOE_BROOKS_STORAGE['ajax_nonce'],
		action: 'cloe_brooks_importer_start_import',
	...
	jQuery.post(CLOE_BROOKS_STORAGE['ajax_url'], data, function(response) {
		...
		if (rez.error === '' || CLOE_BROOKS_STORAGE['importer_ignore_errors']) {
			if (rez.error !== '') 
				CLOE_BROOKS_STORAGE['importer_error_messages'] += '<p class="error_message">' + rez.error + '</p>';
			...
			// Do next action
			if (idx < steps.length) {
				cloe_brooks_importer_do_action(steps, idx);
			} else {
				if (CLOE_BROOKS_STORAGE['importer_error_messages']) {
					jQuery('#trx_importer_progress').removeClass('notice-info').addClass('notice-error').append('<h4>' + CLOE_BROOKS_STORAGE['importer_error_msg'] + '</h4>' + CLOE_BROOKS_STORAGE['importer_error_messages']);
				} else {
